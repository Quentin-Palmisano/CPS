...
public class F16 implements RuleI {
	...
	public ArrayList<EventI> match(EventBaseI eb) {
		...
		for (int i = 0 ; i < eb.numberOfEvents() && (he == null) ; i++) {
			...
			if (e.hasProperty("name") && e.getPropertyValue("name") == FireEventName.NOTIFY_NO_STANDARD_TRUCK_AVAILABLE) {
				he = e;
			}
		}		
		if (he != null) {
	...
	public void act(ArrayList<EventI> matchedEvents, CorrelatorStateI c) throws Exception {
		c.traceRuleTrigger("F16");
		FireCorrelatorStateI fireState = (FireCorrelatorStateI)c;
		fireState.setStandardTruckNotAvailable();
	}
